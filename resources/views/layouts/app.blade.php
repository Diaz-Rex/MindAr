...
</style></head><body><div class="application">@include('layouts.components.sidebar')<main class="content"><div class="topbar"><div><div class="topbar-label">Workspace</div><div class="topbar-name">@yield('title', 'Dashboard')</div></div><div class="user-chip"><div class="user-avatar">{{ strtoupper(substr(Auth::user()->name,0,1)) }}</div><form method="POST" action="{{ route('logout') }}">@csrf<button class="logout-button" type="submit">Sign out</button></form></div></div>@if(session('success'))<div class="success-message">{{ session('success') }}</div>@endif @if(session('error'))<div class="error-message">{{ session('error') }}</div>@endif @if($errors->any())<div class="error-message">@foreach($errors->all() as $error)<div>{{ $error }}</div>@endforeach</div>@endif @yield('content')</main></div>@if(session('zoho_success'))<div id="zoho-success-modal" role="alertdialog" aria-modal="true" aria-labelledby="zoho-success-title" style="position:fixed;inset:0;z-index:9999;display:grid;place-items:center;padding:20px;background:rgba(15,23,42,.55);"><div style="width:min(420px,100%);padding:30px;border:1px solid #bfdbfe;border-radius:20px;background:#fff;text-align:center;box-shadow:0 24px 70px rgba(15,23,42,.3);"><div style="display:grid;width:64px;height:64px;margin:0 auto 16px;place-items:center;border-radius:50%;background:#dcfce7;color:#16a34a;font-size:34px;font-weight:800;">&#10003;</div><h2 id="zoho-success-title" style="margin:0 0 8px;color:#17376e;font-size:24px;">Zoho connected</h2><p style="margin:0 0 22px;color:#64748b;">{{ session('zoho_success') }}</p><button type="button" onclick="document.getElementById('zoho-success-modal').remove()" style="min-width:120px;padding:11px 20px;border:0;border-radius:10px;background:#2563eb;color:#fff;font-weight:700;cursor:pointer;">OK</button></div></div>@endif</body></html>
